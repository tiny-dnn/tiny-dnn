...
TEST(quantization_utils, quantize_down_and_shrink_range) {
   ...
   * to +256.0f, but the actual values present only span -1.0f to 1.0f.
   * We expect the operator to take advantage of this, and rescale the output
   * to fill up the available range in the lower bit depth, and update to the
   * true min and max ranges.
   */
  ...
  Tensor<int32_t> input        = fromVector(input_t);
