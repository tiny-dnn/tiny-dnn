...
TEST(target_cost, get_sample_weight_for_balanced_target_cost) {
    ...
    const serial_size_t              total_samples =
        std::accumulate(class_sample_counts.begin(), class_sample_counts.end(), static_cast<serial_size_t>(0));

    std::vector<float_t> class_weights;
    for (serial_size_t class_sample_count : class_sample_counts) {
        class_weights.push_back(
            get_sample_weight_for_balanced_target_cost(class_count, total_samples, class_sample_count));
    }
...
TEST(target_cost, create_balanced_target_cost_0_5) {
    ...
    for (size_t sample = 0; sample < t.size(); ++sample) {
        ...
        const float_t expected_weight_w_1 =
            t.size() / static_cast<float_t>(label_counts.size() * label_counts[t[sample]]);
...
TEST(target_cost, train_unbalanced_data_1dim) {
    ...
    auto create_net = []() {
        ...
        net << fully_connected_layer<tan_h>(1, 10) << fully_connected_layer<tan_h>(10, 2);
        return net;
    };
    ...
    {                                                    // some simple checks
        const float_t p_label1 = p0 * (1 - p) + p1 * p;  // p(label == 1) = p(label == 1 | in == 0) * p(in
