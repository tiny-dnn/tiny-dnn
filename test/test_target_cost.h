...
TEST(target_cost, get_sample_weight_for_balanced_target_cost) {
  ...
  const cnn_size_t total_samples = std::accumulate(class_sample_counts.begin(),
                                                   class_sample_counts.end(),
...
TEST(target_cost, create_balanced_target_cost_1) {
  ...
  for (size_t sample = 0; sample < t.size(); ++sample) {
    ...
    float_t expected_weight = t.size()
...
TEST(target_cost, create_balanced_target_cost_0_5) {
  ...
  for (size_t sample = 0; sample < t.size(); ++sample) {
    ...
    const float_t expected_weight_w_1 = t.size()
        / static_cast<float_t>(label_counts.size() * label_counts[t[sample]]);
...
TEST(target_cost, train_unbalanced_data_1dim) {
  ...
  { // some simple checks
    const float_t p_label1 = p0 * (1 - p) + p1
        * p; // p(label == 1) = p(label == 1 | in == 0) * p(in == 0) + p(label == 1 | in == 1) * p(in == 1)
    const cnn_size_t n_label1 = std::accumulate(labels.begin(),
                                                labels.end(),
                                                static_cast<cnn_size_t>(0));
  ...
  const auto balanced_cost =
      create_balanced_target_cost(labels); // give higher weight to samples in the minority class
  optimizer1.alpha *= 5;
  optimizer2.alpha *= 5;
  ...
  net_equal_sample_cost.train<mse>(optimizer1,
                                   data,
                                   labels,
                                   10,
                                   100,
                                   ...
                                   true,
                                   CNN_TASK_SIZE);
  net_equal_class_cost.train<mse>(optimizer2,
                                  data,
                                  labels,
                                  10,
                                  100,
  ...
  for (size_t i = 0; i < tnum; i++) {
    ...
    const label_t
        actual_equal_sample_cost = net_equal_sample_cost.predict_label(input);
    const label_t
        actual_equal_class_cost = net_equal_class_cost.predict_label(input);
