...
TEST(batchnorm, gradient_check1) {
  ...
  std::vector<Tensor<> *> in_data, out_data, in_grad, out_grad;
  ...
  Tensor<> outd_t(outd), ing_t(ing), outg_t(outg);
  in_grad.push_back(&ing_t);
  out_grad.push_back(&outg_t);
  out_data.push_back(&outd_t);
