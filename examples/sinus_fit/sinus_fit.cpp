// this example show how to use tiny-dnn library to fit data, by learning a sinus function
...
int main()
{
    ...
    //create learning data, X is the input set and sinX is the expected output set
    vector<vec_t> X;
    vector<vec_t> sinusX;
    for(float x=-3.1416f;x<3.1416f;x+=0.1f) //learn and test on a period
    {
        vec_t vx= {x};
        vec_t vsinx= {sinf(x)};
    ...
    auto on_enumerate_epoch = [&]()
    {
        iEpoch++;
        if(iEpoch % 100)
            return; //compute loss and disp 1/100 of the time
