...
class timer {
 public:
  ...
  float_t elapsed() {
    return std::chrono::duration_cast<std::chrono::duration<float_t>>(std::chrono::high_resolution_clock::now() - t1)
        .count();
  }
...
class progress_display {
 ...
 private:
  ...
  void display_tic() {
    ...
    size_t tics_needed = static_cast<size_t>((static_cast<double>(_count) / _expected_count) * 50.0);
    do {
      m_os << '*' << std::flush;
