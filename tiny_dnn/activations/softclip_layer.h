...
class softclip_layer : public activation_layer {
 public:
  ...
  softclip_layer(const shape3d &in_shape,
    ...
    alpha_invert_ = float_t(1) / alpha_;
    threshold_digits_= std::numeric_limits<float_t>::digits10;
    auto e = epsilon_value(); 
    threshold_left_ = alpha_invert_* log( ( exp( alpha_ *             e )  - float_t(1) )/( float_t(1) - exp( alpha_ * (            e  -float_t(1) ) ) ) );
    threshold_right_= alpha_invert_* log( ( exp( alpha_ * (float_t(1)-e) ) - float_t(1) )/( float_t(1) - exp( alpha_ * ((float_t(1)-e) -float_t(1) ) ) ) );
  }
  ...
  float_t epsilon_value() const { return pow(float_t(10), -threshold_digits_ ); }
  ...
  void backward_activation(const vec_t &x,
    ...
    for (size_t j = 0; j < x.size(); j++) {
      ...
      float_t left  = exp_ax/(float_t(1)+exp_ax);
  ...
  float_t threshold_digits_=7; //!< Internal variable. Number of digits ( nines or zeros ) to calculate before function becom constant.
