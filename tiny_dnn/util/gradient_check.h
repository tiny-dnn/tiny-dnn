...
float_t numeric_gradient(layer &layer,
  ...
  std::for_each(out_grads.begin(), out_grads.end(),
                [&](tensor_t &i) { fill_tensor(i, 0.0); });
  std::for_each(out_data.begin(), out_data.end(),
                [&](tensor_t &i) { fill_tensor(i, 0.0); });
  ...
  in_data[in_edge][0][in_pos] = prev_in;
  layer.forward_propagation(in_data_, out_data_);
  in_data[in_edge][0][in_pos] = prev_in;
 ...
 */
float_t computed_gradient(layer &layer,
  ...
  std::vector<tensor_t *> out_data_ = tensor2ptr(out_data);
  std::for_each(in_grads.begin(), in_grads.end(),
                [&](tensor_t &i) { fill_tensor(i, 0.0); });
  std::for_each(out_grads.begin(), out_grads.end(),
                [&](tensor_t &i) { fill_tensor(i, 0.0); });
  std::vector<tensor_t *> out_grads_ = tensor2ptr(out_grads);
 ...
 * http://cs231n.github.io/neural-networks-3/#gradcheck
 * @param computed_gradient
 * @param numeric_gradient
 * @return the relative error.
 */
float_t relative_error(const float_t computed_gradient,
                       const float_t numeric_gradient) {
  float_t max =
    std::max(std::abs(computed_gradient), std::abs(numeric_gradient));
  if (max == 0) {
    return 0;
  } else {
    return std::abs(computed_gradient - numeric_gradient) / max;
  }
}
