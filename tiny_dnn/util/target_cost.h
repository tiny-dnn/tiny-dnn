...
inline std::vector<serial_size_t> calculate_label_counts(const std::vector<label_t>& t) {
    ...
    assert(std::accumulate(label_counts.begin(),
                           label_counts.end(),
                           static_cast<serial_size_t>(0)) == t.size());
    return label_counts;
}
...
inline float_t get_sample_weight_for_balanced_target_cost(serial_size_t classes,
                                                          serial_size_t total_samples,
                                                          serial_size_t this_class_samples) {
...
inline std::vector<vec_t> create_balanced_target_cost(const std::vector<label_t>& t,
                                                      float_t w = 1.0) {
    ...
    for (serial_size_t i = 0; i < total_sample_count; ++i) {
        ...
        const float_t balanced_weight = get_sample_weight_for_balanced_target_cost(class_count,
                                                                                   total_sample_count,
                                                                                   label_counts[t[i]]);
