...
inline float_t get_sample_weight_for_balanced_target_cost(cnn_size_t classes,
                                                          cnn_size_t total_samples,
...
inline std::vector<vec_t> create_balanced_target_cost(const std::vector<label_t> &t,
  ...
  for (cnn_size_t i = 0; i < total_sample_count; ++i) {
    ...
    const float_t
        sample_weight = w * balanced_weight + (1 - w) * unbalanced_weight;
