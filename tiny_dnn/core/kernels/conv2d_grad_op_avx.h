...
inline void accumulate_db(const index3d<serial_size_t>& out, const std::vector<float, Allocator>& curr_delta,
...
// float ver
template <typename Allocator>
inline void accumulate_dw(const core::conv_params& params, const std::vector<float, Allocator>& prev_out,
                          const std::vector<float, Allocator>& curr_delta, std::vector<float, Allocator>& dW,
                          std::vector<float, Allocator>& db) {
    ...
    if (w_stride > 1) {
      for (serial_size_t inc = 0; inc < in.depth_; ++inc) {
        for (serial_size_t outc = 0; outc < out.depth_; outc++) {
          ...
          for (size_t wy = 0; wy < 5;  // weight.height_
               ++wy) {
            for (size_t wx = 0; wx < 5;  // weight.width_
                 ++wx, ++widx) {
              serial_size_t prev_out_idx =
                  in_padded.get_index(static_cast<serial_size_t>(wx), static_cast<serial_size_t>(wy), inc);
      ...
      for (serial_size_t inc = 0; inc < in.depth_; ++inc) {
        for (serial_size_t outc = 0; outc < out.depth_; outc++) {
          ...
          for (size_t wy = 0; wy < 5;  // weight.height_
               ++wy) {
            size_t prev_out_idx = in_padded.get_index(0, static_cast<serial_size_t>(wy), inc);
            ...
            _mm_storeu_ps(pdw + wy * 5, _mm_add_ps(_mm_loadu_ps(pdw + wy * 5), hsum4x256_ps(sum0, sum1, sum2, sum3)));
            _mm_store_ss(pdw + wy * 5 + 4, _mm_add_ss(_mm_load_ss(pdw + wy * 5 + 4), hsum256_ps(sum4)));
          }  // for wy
        }    // for outc
      }      // for inc
    } else if (nblocks > 1 && remainder != 0) {
      for (serial_size_t inc = 0; inc < in.depth_; ++inc) {
        for (serial_size_t outc = 0; outc < out.depth_; outc++) {
          ...
          for (size_t wy = 0; wy < 5;  // weight.height_
            ...
            const float* pb = delta;
            sum0 = sum1 = sum2 = sum3 = sum4 = _mm256_setzero_ps();
            for (size_t y = 0; y < out_height; ++y, pa += prevo_delta, pb += out_width) {
              // vectorize::dot
              __m256 a0 = _mm256_loadu_ps(pa + 0);
              __m256 a1 = _mm256_loadu_ps(pa + 1);
              __m256 a2 = _mm256_loadu_ps(pa + 2);
...
// float ver
template <typename Allocator>
void avx_conv2d_5x5_back_kernel_one(const core::conv_params& params, const std::vector<float, Allocator>& prev_out,
                                    const std::vector<float, Allocator>& W, std::vector<float, Allocator>& dW,
                                    std::vector<float, Allocator>& db, std::vector<float, Allocator>& curr_delta,
  ...
  if (w_stride == 1 && out.width_ >= 4) {
    ...
    for (serial_size_t inc = 0; inc < in.depth_; ++inc, pdelta_dst_org += in_padded_area) {
      ...
      // ---3 3333
      // ---4 4444
      __m256 new_sum0 = _mm256_blend_ps(_mm256_setzero_ps(), sum0, 0x1F /* 0b00011111 */
                                        );
      __m256 new_sum1 = _mm256_blend_ps(
          _mm256_setzero_ps(), _mm256_or_ps(rightShift<20>(sum0), leftShift<12>(sum1)), 0x1F /* 0b00011111 */
          );
      __m256 new_sum2 = _mm256_blend_ps(_mm256_setzero_ps(), rightShift<8>(sum1), 0x1F /* 0b00011111 */
                                        );
      __m256 new_sum3 = _mm256_blend_ps(
          _mm256_setzero_ps(), _mm256_or_ps(rightShift<28>(sum1), leftShift<4>(sum2)), 0x1F /* 0b00011111 */
          );
      __m256 new_sum4 = _mm256_blend_ps(
          _mm256_setzero_ps(), _mm256_set_m128(sum3, _mm256_extractf128_ps(sum2, 1)), 0x1F /* 0b00011111 */
          );
...
void avx_conv2d_5x5_back_kernel(const core::conv_params& params,
                                ...
                                const std::vector<float, Allocator>& W, std::vector<std::vector<float, Allocator>>& dW,
  ...
  for_i(prev_out.size(), [&](int sample) {
    avx_conv2d_5x5_back_kernel_one(params, prev_out[sample], W, dW[sample], db[sample], curr_delta[sample],
                                   &prev_delta[sample]);
  });
}
...
inline void conv2d_grad_op_avx(const tensor_t& prev_out, const vec_t& W, tensor_t& dW, tensor_t& db,
                               tensor_t& curr_delta, tensor_t& prev_delta, const core::conv_params& params,
  ...
  conv2d_op_internal(prev_out, W, dW, db, curr_delta, prev_delta, params, layer_parallelize);
}
