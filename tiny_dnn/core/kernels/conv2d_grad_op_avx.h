    ...
    if (out.width_ == 1 && out.height_ == 1) {
        ...
        for (size_t i = 0; i < nblocks; ++i) {
            _mm256_storeu_ps(&db[i * 8],
                             _mm256_add_ps(_mm256_loadu_ps(&db[i * 8]), _mm256_loadu_ps(&curr_delta[i * 8])));
        }
                ...
                for (serial_size_t outc = 0; outc < out.depth_; outc++) {
                    ...
                    for (size_t wy = 0; wy < 5;  // weight.height_
                        ...
                        for (size_t wx = 0; wx < 5;  // weight.width_
                             ++wx, ++widx) {
                            serial_size_t prev_out_idx = in_padded.get_index(static_cast<serial_size_t>(wx),
                ...
                for (serial_size_t outc = 0; outc < out.depth_; outc++) {
                    ...
                    for (size_t wy = 0; wy < 5;  // weight.height_
                        ...
                        _mm_storeu_ps(pdw + wy * 5,
                                      _mm_add_ps(_mm_loadu_ps(pdw + wy * 5), hsum4x256_ps(sum0, sum1, sum2, sum3)));
