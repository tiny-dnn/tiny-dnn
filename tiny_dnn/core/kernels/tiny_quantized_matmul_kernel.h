...
void tiny_quantized_matmul(const std::vector<T1> &   a,
    ...
    if (std::is_same<T1, uint8_t>() && std::is_same<T2, uint8_t>() && std::is_same<Toutput, int32_t>() &&
        ...
        if (transpose_a_) {
            if (transpose_b_) {
                gemmlowp_multiply<true, true, false>(a_data, b_data, c_data, m, n, k, offset_a, offset_b, lda, ldb,
                                                     ldc);
            } else {
                gemmlowp_multiply<true, false, false>(a_data, b_data, c_data, m, n, k, offset_a, offset_b, lda, ldb,
                                                      ldc);
            }
        } else {
            if (transpose_b_) {
                gemmlowp_multiply<false, true, false>(a_data, b_data, c_data, m, n, k, offset_a, offset_b, lda, ldb,
                                                      ldc);
            } else {
                gemmlowp_multiply<false, false, false>(a_data, b_data, c_data, m, n, k, offset_a, offset_b, lda, ldb,
                                                       ldc);
            }
        }
    } /*else {
