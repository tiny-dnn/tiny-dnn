...
void gemmlowp_multiply(const uint8_t* a_data, const uint8_t* b_data, int32_t* c_data, int m, int n, int k, int offset_a,
  ...
  static const gemmlowp::MapOrder LhsOrder = !TransposeA ? gemmlowp::MapOrder::RowMajor : gemmlowp::MapOrder::ColMajor;
  static const gemmlowp::MapOrder RhsOrder = !TransposeB ? gemmlowp::MapOrder::RowMajor : gemmlowp::MapOrder::ColMajor;
  gemmlowp::MatrixMap<const std::uint8_t, LhsOrder> lhs(a_data_as_uint8, m, k, lda);
  gemmlowp::MatrixMap<const std::uint8_t, RhsOrder> rhs(b_data_as_uint8, k, n, ldb);
  gemmlowp::MatrixMap<std::int32_t, ResultOrder> result(c_data_as_int32, m, n, ldc);
...
void tiny_quantized_matmul(const std::vector<T1>& a, const std::vector<T2>& b, std::vector<Toutput>& c,
                           const std::vector<size_t> shape_all, const int32_t offset_a, const int32_t offset_b,
                           const int32_t offset_c, const int32_t mult_c, const int32_t shift_c) {
  ...
  if (std::is_same<T1, uint8_t>() && std::is_same<T2, uint8_t>() && std::is_same<Toutput, int32_t>() &&
      (offset_c == 0) && (mult_c == 1) && (shift_c == 0) && (transpose_c == false)) {
    if (transpose_a_) {
      if (transpose_b_) {
        gemmlowp_multiply<true, true, false>(a_data, b_data, c_data, m, n, k, offset_a, offset_b, lda, ldb, ldc);
      } else {
        gemmlowp_multiply<true, false, false>(a_data, b_data, c_data, m, n, k, offset_a, offset_b, lda, ldb, ldc);
      }
    } else {
      if (transpose_b_) {
        gemmlowp_multiply<false, true, false>(a_data, b_data, c_data, m, n, k, offset_a, offset_b, lda, ldb, ldc);
      } else {
        gemmlowp_multiply<false, false, false>(a_data, b_data, c_data, m, n, k, offset_a, offset_b, lda, ldb, ldc);
      }
    }
  } /*else {
