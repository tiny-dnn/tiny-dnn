...
#include "tiny_dnn/core/kernels/conv2d_error_op.h"
...
class convolutional_error_layer : public feedforward_layer<Activation> {
 public:
  ...
  **/
  convolutional_error_layer(serial_size_t in_width,
    ...
    : convolutional_error_layer(in_width,
                          in_height,
                          window_size,
                          window_size,
                          in_channels,
  ...
  **/
  ...
  // move constructor
  convolutional_error_layer(convolutional_error_layer &&other)  // NOLINT
    : Base(std::move(other)),
 ...
 private:
  ...
  void init_backend(const backend_t backend_type) {
    ...
    if (backend_type == backend_t::internal ||
      ...
      kernel_fwd_.reset(new Conv2dErrorOp(ctx));
