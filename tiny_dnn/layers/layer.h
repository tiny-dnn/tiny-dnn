...
/// visualization.
  ...
  /* @brief Allocates data in the computational graph and reset layer parameters
   * if
   * it's needed or the data is not already initialized.
   *
   * @param reset_parameters Boolean value to force to reset the parameters.
   * Weights will be automatically reset if the are not initialized.
   *
   */
  void setup(bool reset_parameters) {
    ...
    if (reset_parameters || !initialized_) {
      init_parameters();
    }
  }
  ...
  void init_parameters() {
    ...
    for (size_t i = 0; i < parameters_.size(); i++) {
      switch (parameters_[i]->get_param_type()) {
        // fill parameter of weight type
        case param_type::weight:
          weight_init_->fill(get_ith_parameter(i)->get_data(), fan_in_size(),
                             fan_out_size());
          break;
        // fill parameter of bias type
        case param_type::bias:
  ...
  void clear_grads() {
    for (size_t i = 0; i < parameters_.size(); i++) {
      get_ith_parameter(i)->clear_grads();
    }
  }
  ...
  void update_weight(optimizer *o, serial_size_t batch_size) {
    ...
    for (serial_size_t i = 0;
         i < static_cast<serial_size_t>(parameters_.size()); i++) {
      if (trainable() && parameters_[i]->is_trainable()) {
        vec_t &target = *get_ith_parameter(i)->get_data();
        get_ith_parameter(i)->merge_grads(&diff);
  ...
  bool has_same_weights(const layer &rhs, float_t eps) const {
    ...
    for (size_t i = 0; i < w1.size(); i++) {
      ...
      for (size_t j = 0; j < w1[i]->size(); j++) {
        if (std::abs(*(w1[i]->data_at(j)) - *(w2[i]->data_at(j))) > eps)
          return false;
      }
    }
    return true;
  ...
  virtual void set_sample_count(serial_size_t sample_count) {
    ...
    for (size_t i = 0; i < parameters_.size(); i++) {
      get_ith_parameter(i)->resize_grad(sample_count);
    }
  ...
  void add_parameter(serial_size_t width,
    ...
    parameters_.push_back(std::make_shared<parameter>(
      width, height, depth, n_fmaps, type, trainable));
  }
  ...
  std::shared_ptr<parameter> get_ith_parameter(size_t i) const {
    return static_cast<std::shared_ptr<parameter>>(parameters_[i]);
  }
