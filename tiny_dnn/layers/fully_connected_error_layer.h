...
class fully_connected_error_layer : public feedforward_layer<Activation> {
 public:
   ...
   **/
  fully_connected_error_layer(serial_size_t in_dim,
  ...
  // move constructor
  fully_connected_error_layer(fully_connected_error_layer &&other)
    : Base(std::move(other)),
 ...
 protected:
  ...
  void init_backend(backend_t backend_type) {
    ...
    if (backend_type == backend_t::internal || backend_type == backend_t::avx ||
      ...
      kernel_fwd_.reset(new FullyConnectedErrorOp(ctx));
