...
graph_visualizer vizualizer(net, "graph_name");

visualizer.generate(std::cout);

std::ofstream ofs("graph.txt");
visualizer.generate(ofs);
```

Once we get dot language model, we can easily get an image by graphviz:

```cmd
dot -Tgif graph.txt -o graph.gif
```
...
### <a name="change-the-number-of-threads-while-training"></a>change the number of threads while training
